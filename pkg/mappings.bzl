...
# TODO(VCBAZ-2070): This directory must be explicitly excluded from ownership in
# this RPM, as it's owned by another package.  Since the contents of this are
# created from a directory and there's no mechanism for consistently handling
# TreeArtifacts (directories) in rules_pkg, this must be worked around in the
# spec file.
